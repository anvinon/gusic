...
def other_artist(request):
    ...
    selected_id = Artist.objects.filter(name__iexact=selected_artist).values_list('id', flat = True)[0]
    # ジャンルテーブルをジャンル名で検索
    genre_id= Genre.objects.filter(name__iexact=genre).values_list('id', flat = True)[0]
    # ジャンルとアーティストの外部テーブルを検索し、特定のジャンルに属する複数のアーティストのIDを取得
    artist_ids= ArtistAndGenre.objects.filter(genre_id=genre_id).values_list('artist_id', flat = True)
...
def jump(request):
    ...
    jump_url = Songs.objects.filter(title__iexact=song).values_list('url', flat = True)[0]
